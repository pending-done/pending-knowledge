...
개발을 시작한 뒤 웹사이트를 돌아다니다보면 흔하게 보이는 기능들이 있다. `캘린더`, `페이지네이션`, `모달` 등... 전부 너무 흔하게 들어가있는 기능들이라 어려울거라 생각도 못한 기능들이다. 하지만 곰곰히 생각해보면 대체 어떻게 한거지? 싶은 것들이 아직 많다. 

캘린더는 그 중 하나였고 마침 캘린더를 개발할 기회가 생겨 라이브러리를 사용하지 않고 구현해보았다. 개발하기 전에는 감도 잡히지 않던 기술이지만 지금은 머리속에 어느 정도 정리가 된 상태고 매우 뿌듯하고 개운해서 기분이 좋다 ㅎ. 앞으로도 라이브러리를 사용하지 않고 여러 가지 기능들을 만들어 볼 생각이다. (아마 토스트가 첫 그거일듯) 물론 캘린더 기능부터 며칠 내로 업그레이드 해서 다음 포스팅을 진행할 예정이다.

### 캘린더 업그레이드 체크리스트

- 실제 데이터 기준으로 동작되도록 변경 (선행 작업 이후 진행예정)
- 현재의 이 조잡한 디자인을 빨리 바꾸고 싶지만 실제 데이터가 생성된 후 진행 예정
- 추가 기능 개발 (필터링 등)

## 라이브러리 없이 캘린더 구현하기
[#1 기본 UI 구현](./calendar)
[#2 현재 글 - 일정 표시 구현](./calendar-2)