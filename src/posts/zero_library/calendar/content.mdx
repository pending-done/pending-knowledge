...
thumbnail: /zero_library/calendar/calendar_1.png
---
...
이번에 구현하려고 하는 기능은 날짜 확인은 기본이고, 일정까지 확인할 수 있도록 구현할 예정이다.
...
월 기준 ***첫 번째로 표시되는 일자*** 와 ***마지막으로 표시되는 일자*** 를 어떻게 구할 것인가? 이게 **기본 캘린더 UI**를 만들 때 가장 골치아팠던 것 중 하나이다. (물론 내 기준임 ...) 이 부분은 다음과 같이 구현했다. 
1. 현재 **월의 첫 일자 기준** `해당 주의 시작(dayStart)`이 되는 일자를 구한다.
    - 예시 사진을 보면 이전 월에 해당하는 26일 부터 시작 하는 것을 볼 수 있다.
2. 현재 **월의 마지막 일자 기준** `해당 주의 마지막(dayEnd)`이 되는 일자를 구한다.
    - 예시 사진을 보면 다음 월에 해당하는 6일 까지 출력 되는 것을 볼 수 있다.

즉, `dayStart` 부터 `dayEnd` 까지 출력해주면 된다.
...
버튼을 생성하고 `Calendar` 컴포넌트에서 Props로 전달한 `prevMonth`, `nextMonth`를  `onClick` 이벤트에 넣어준다. (버튼 디자인은 못본걸로 해주세요.)
